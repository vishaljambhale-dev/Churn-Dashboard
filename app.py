...
def load_trade_data(file):
    ...
    try:
        # Explicitly tell Pandas which engine to use based on the extension
        ...
        # FALLBACK: If it throws a ValueError, it is likely a "fake" .xls file 
        # (an HTML table generated by a broker system).
        try:
            file.seek(0) # Reset the file pointer to the beginning
            tables = pd.read_html(file)
            return tables[0] if tables else pd.DataFrame()
        except Exception as e:
            st.error(f"Could not read the file. Please ensure it is a valid Excel or tabular file. Error: {e}")
            return pd.DataFrame()
